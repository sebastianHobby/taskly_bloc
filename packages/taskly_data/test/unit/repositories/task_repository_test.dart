...
void main() {
  ...
  group('TaskRepository', () {
    ...
    testSafe('watchAllCount uses default query when omitted', () async {
      final db = createAutoClosingDb();
      final repo = TaskRepository(
        driftDb: db,
        occurrenceExpander: _FakeOccurrenceExpander(),
      ...
      await repo.create(name: 'Task A');
      await repo.create(name: 'Task B');

      final count = await repo.watchAllCount().first;
    ...
    testSafe('getById returns task with project and override values', () async {
      final db = createAutoClosingDb();
      final repo = TaskRepository(
        driftDb: db,
        occurrenceExpander: _FakeOccurrenceExpander(),
      ...
      expect(task!.overridePrimaryValueId, equals('v2'));
      expect(task.overrideSecondaryValueId, equals('v3'));
      expect(task.projectId, equals('p1'));
    });

    testSafe('getAll defaults to TaskQuery.all when omitted', () async {
      final db = createAutoClosingDb();
      final repo = TaskRepository(
        driftDb: db,
        occurrenceExpander: _FakeOccurrenceExpander(),
    ...
    testSafe('bulk reschedule methods return zero for empty ids', () async {
      ...
      final deadlineUpdated = await repo.bulkRescheduleDeadlines(
        taskIds: const [],
        deadlineDate: DateTime.utc(2026, 2, 11),
      );
      final startUpdated = await repo.bulkRescheduleStarts(
        taskIds: const [],
        startDate: DateTime.utc(2026, 2, 11),
      );

      expect(deadlineUpdated, equals(0));
      expect(startUpdated, equals(0));
    });
