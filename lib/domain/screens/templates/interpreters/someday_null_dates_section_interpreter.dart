...
import 'package:taskly_bloc/domain/screens/templates/params/someday_null_dates_section_params.dart';
...
/// Section template for Someday: tasks/projects with no start/deadline dates.
///
/// Also decorates results with "in focus" membership from today's allocation
/// snapshot, so the UI can show a focus icon when an item is in focus.
class SomedayNullDatesSectionInterpreter
    implements SectionTemplateInterpreter<SomedayNullDatesSectionParams> {
  ...
  @override
  String get templateId => SectionTemplateId.somedayNullDates;

  static const String _focusTaskIdsKey = 'focusTaskIds';
  static const String _focusProjectIdsKey = 'focusProjectIds';

  @override
  Stream<SectionDataResult> watch(SomedayNullDatesSectionParams params) {
    ...
    return Rx.combineLatest3(
      ...
      ) => _buildResult(tasks: tasks, projects: projects, snapshot: snapshot),
    );
  }

  @override
  Future<SectionDataResult> fetch(SomedayNullDatesSectionParams params) async {
  ...
  DateTime _sortKeyFor(ScreenItem item) {
    return switch (item) {
      ...
      // Should not occur for this template.
      _ => DateTime.utc(0),
    };
  }
