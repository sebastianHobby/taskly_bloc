...
/// A widget that displays value chips with colored icons.
///
/// In truncated mode (default), shows values in up to 2 lines with a "+X more"
/// indicator if there are more values than can fit.
///
/// In expanded mode, shows all values without truncation.
class TruncatedValueChips extends StatelessWidget {
  const TruncatedValueChips({
  ...
  /// When false, all values are shown.
  final bool truncate;
  ...
  Widget build(BuildContext context) {
    ...
    if (!truncate) {
      ...
      return Wrap(
        ...
        children: values.map((value) => ValueChip(value: value)).toList(),
      );
    }
    ...
    return _TwoLineWrap(
      ...
      runSpacing: runSpacing,
      values: values,
      chipBuilder: (value) => ValueChip(value: value),
...
/// Internal widget that displays values in up to a specified number of lines.
class _TwoLineWrap extends StatefulWidget {
  ...
  final double runSpacing;
  final List<Value> values;
  final Widget Function(Value) chipBuilder;
...
class _TwoLineWrapState extends State<_TwoLineWrap> {
  ...
  void didUpdateWidget(_TwoLineWrap oldWidget) {
    ...
    if (oldWidget.values.length != widget.values.length ||
        !_listsEqual(oldWidget.values, widget.values)) {
      _measured = false;
      _initKeys();
    }
  }

  bool _listsEqual(List<Value> a, List<Value> b) {
  ...
  void _measureChips() {
    ...
    if (mounted && count != _visibleCount) {
      setState(() {
        _visibleCount = count > 0 ? count : widget.values.length;
        _measured = true;
      });
    }
  }
  ...
  @override
  Widget build(BuildContext context) {
    final effectiveVisible = _measured ? _visibleCount : widget.values.length;
    final remaining = widget.values.length - effectiveVisible;
          ...
          for (var i = 0; i < widget.values.length; i++)
            if (!_measured || i < effectiveVisible)
              KeyedSubtree(
                ...
                child: widget.chipBuilder(widget.values[i]),
              ),
