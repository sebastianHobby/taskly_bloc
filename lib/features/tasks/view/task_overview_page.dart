...
class TaskOverviewPage extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return BlocProvider(
      ...
      child: TaskOverviewView(taskRepository: taskRepository),
    );
  }
}
      ...
      body: BlocBuilder<TaskOverviewBloc, TaskOverviewState>(
        builder: (context, state) {
          return state.when(
            ...
            loaded: (tasks) => TasksListView(tasks: tasks, taskRepository: taskRepository),
            error: (message, _) => Center(child: Text(message)),
          );
        },
      ),
      floatingActionButton: AddTaskFab(taskRepository: taskRepository),
    );
  }
}
