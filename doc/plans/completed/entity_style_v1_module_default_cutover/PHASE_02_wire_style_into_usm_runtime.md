...
Summary:
- Threaded `EntityStyleV1` through the USM runtime (`SectionVm`) for entity-rendering sections.
- Added params-level `EntityStyleOverrideV1?` where needed and removed `StylePackV2` from the typed params surface.
- Updated system screen specs and call sites so the domain compiles without `StylePackV2`.
