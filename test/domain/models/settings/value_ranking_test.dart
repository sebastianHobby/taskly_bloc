...
void main() {
  ...
  group('ValueRanking', () {
    ...
    group('weightForLabel', () {
      test('returns weight for existing label', () {
        const ranking = ValueRanking(
          items: [
            ValueRankItem(labelId: 'label1', weight: 3),
            ValueRankItem(labelId: 'label2', weight: 7),
          ],
        );

        expect(ranking.weightForLabel('label1'), 3);
        expect(ranking.weightForLabel('label2'), 7);
      });

      test('returns null for non-existing label', () {
        const ranking = ValueRanking(
          items: [ValueRankItem(labelId: 'label1', weight: 3)],
        );

        expect(ranking.weightForLabel('nonExistent'), isNull);
      });

      test('returns null for empty ranking', () {
        const ranking = ValueRanking();

        expect(ranking.weightForLabel('anyLabel'), isNull);
      });
    });

    group('upsertItem', () {
      test('adds new item when not existing', () {
        const ranking = ValueRanking(
          items: [ValueRankItem(labelId: 'existing', weight: 5)],
        );
        const newItem = ValueRankItem(labelId: 'new', weight: 8);

        final updated = ranking.upsertItem(newItem);

        expect(updated.items.length, 2);
        expect(updated.weightForLabel('existing'), 5);
        expect(updated.weightForLabel('new'), 8);
      });

      test('updates existing item', () {
        const ranking = ValueRanking(
          items: [
            ValueRankItem(labelId: 'existing', weight: 5),
            ValueRankItem(labelId: 'another', weight: 3),
          ],
        );
        const updatedItem = ValueRankItem(labelId: 'existing', weight: 10);

        final updated = ranking.upsertItem(updatedItem);

        expect(updated.items.length, 2);
        expect(updated.weightForLabel('existing'), 10);
        expect(updated.weightForLabel('another'), 3);
      });

      test('preserves order when updating', () {
        const ranking = ValueRanking(
          items: [
            ValueRankItem(labelId: 'first', weight: 1),
            ValueRankItem(labelId: 'second', weight: 2),
            ValueRankItem(labelId: 'third', weight: 3),
          ],
        );
        const updatedItem = ValueRankItem(labelId: 'second', weight: 99);

        final updated = ranking.upsertItem(updatedItem);

        expect(updated.items[0].labelId, 'first');
        expect(updated.items[1].labelId, 'second');
        expect(updated.items[1].weight, 99);
        expect(updated.items[2].labelId, 'third');
      });

      test('adds item to empty ranking', () {
        const ranking = ValueRanking();
        const newItem = ValueRankItem(labelId: 'first', weight: 5);
    ...
    group('removeItem', () {
      test('removes existing item', () {
        const ranking = ValueRanking(
          items: [
            ValueRankItem(labelId: 'keep', weight: 5),
            ValueRankItem(labelId: 'remove', weight: 8),
          ],
        );

        final updated = ranking.removeItem('remove');

        expect(updated.items.length, 1);
        expect(updated.items[0].labelId, 'keep');
      });

      test('does nothing for non-existing item', () {
        const ranking = ValueRanking(
          items: [ValueRankItem(labelId: 'keep', weight: 5)],
        );

        final updated = ranking.removeItem('nonExistent');

        expect(updated.items.length, 1);
        expect(updated.items[0].labelId, 'keep');
      });

      test('returns empty ranking when removing last item', () {
        const ranking = ValueRanking(
          items: [ValueRankItem(labelId: 'only', weight: 5)],
        );

        final updated = ranking.removeItem('only');
